...
async def safe_send_json(websocket: WebSocket, data: dict) -> bool:
    """Safely send a JSON message over WebSocket if connected."""
    try:
            ...
            if data.get("type") == "ping":
                ...
                if video_url:
                    ...
                    async def receive_listener():
                        try:
                            while not stop_event.is_set() and websocket.client_state == WebSocketState.CONNECTED:
                                msg_text = await websocket.receive_text()
                                msg_data = json.loads(msg_text)
                                if msg_data.get("type") == "ping":
                                    await safe_send_json(websocket, {"type": "pong"})
                                elif msg_data.get("type") == "stop":
                                    stop_event.set()
                                    break
                        except (WebSocketDisconnect, RuntimeError):
                            stop_event.set()
                        except Exception as err:
                            logging.debug(f"Receive listener closed: {err}")
                            stop_event.set()

                    listener_task = asyncio.create_task(receive_listener())
                    try:
                        await process_video_stream(websocket, video_url, connection_id, stop_event)
                    finally:
                        stop_event.set()
...
async def process_video_stream(websocket: WebSocket, video_url: str, connection_id: str, stop_event: Optional[asyncio.Event] = None):
    cap = None
    try:
        ...
        if not await safe_send_json(websocket, {
            ...
            logging.info(f"Client disconnected before video processing started: {connection_id}")
            return
        ...
        while cap.isOpened():
            ...
            if stop_event and stop_event.is_set():
                logging.info(f"Stop signal received during frame processing: {connection_id}")
                break
            ...
            if not sent:
                logging.info(f"Client disconnected while sending frame: {connection_id}")
                break
            ...
            if frame_count % 30 == 0:
                ...
                if not sent:
                    logging.info(f"Client disconnected while sending progress: {connection_id}")
                    break
        ...
        if not (stop_event and stop_event.is_set()) and websocket.client_state == WebSocketState.CONNECTED:
            location = "Unknown location"
